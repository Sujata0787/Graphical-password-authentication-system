...
class login:
    def __init__(self,root):
        ...
        self.bg=Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/loginpage.jpg")
        self.resizeA=self.bg.resize((1525,800), Image.Resampling.LANCZOS)
...
#****************************************First level of text base Security Page ***********************************
        ...
        title=Label(Frame_login,text="Login Here",font=("time new roman",25,"bold"),fg="#d77337",bg="pink").place(x=170,y=30)
        desc = Label(Frame_login, text=" Text Base Password Login", font=("time new roman", 12, "bold"), fg="#d25d17",
                      bg="pink").place(x=100, y=90)
        lbl_user= Label(Frame_login, text="Email", font=("time new roman", 15,), fg="black",
                      bg="pink").place(x=90, y=140)
        self.txt_user=Entry(Frame_login,font=("time new roman", 15),bg="white")
        self.txt_user.place(x=90,y=170,width=300,height=35)

        lbl_password= Label(Frame_login, text="Password", font=("time new roman", 15,), fg="black",
                         bg="pink").place(x=90, y=210)
        self.txt_password = Entry(Frame_login, font=("time new roman", 15), bg="white", show='*')
        self.txt_password.place(x=90, y=240, width=300, height=35)

        forget=Button(Frame_login,command=self.forget_password,cursor="hand2",text="Forget Password?", bg="pink",fg="black",bd=0,
                      font=("time new roman",12)).place(x=130,y=280)
        Login = Button(Frame_login, command=self.login_function,text="Login", bg="blue", fg="white",
        ...
        Back= Button(Frame_login, command=self.Back, text="Back", bg="#d77337", fg="white",
                       font=("time new roman", 12, "bold")).place(x=170, y=450, width=150, height=40)
        register_label = Label(Frame_login, text="If you have not registered, Go Back to Home Page!", font=("time new roman", 12),
                        fg="red",bg="pink").place(x=100, y=400)
    ...
    def reset_password(self):
        if self.cmb_question.get()=="" or self.new_password.get()=="" or self.val_answer.get()=="":
            ...
            try:
                ...
                con = pymysql.connect(host="localhost", user="root", password="miki", database="employee3")
                cur = con.cursor()
                cur.execute("select * from student70 where email=%s and question=%s and answer=%s", (self.txt_user.get(),self.cmb_question.get(),self.val_answer.get()))
                row = cur.fetchone()
                if row == None:
                    ...
                    cur.execute("update student70 set password=%s where email=%s",(self.reset_encrypted_text_password,self.txt_user.get()))
                    con.commit()
                    con.close()
                    self.engine.say('Your password has been saved')
                    self.engine.runAndWait()
                    messagebox.showinfo("Success","Your password has been saved",parent=self.root2)
    ...
    def forget_password(self):
        if self.txt_user.get()=="":
            self.engine.say('Please Enter email address to to move forward')
            self.engine.runAndWait()
            messagebox.showerror("Error","Please Enter email address to move forward",parent=self.root)
        else:
            try:
                con = pymysql.connect(host="localhost", user="root", password="miki", database="employee3")
                cur = con.cursor()
                cur.execute("select * from student70 where email=%s",self.txt_user.get())
                row = cur.fetchone()
                if row == None:
    ...
    def login_function(self):
        if  self.txt_user.get()=="":
            ...
            try:
                ...
                con=pymysql.connect(host="localhost",user="root",password="miki",database="employee3")
                cur=con.cursor()
                cur.execute("select * from student70 where email=%s and password=%s",(self.txt_user.get(),self.encrypted_password))
                row=cur.fetchone()
                if row==None:
...
# ********************************Secend level of colour base Security Page*************************************
    ...
    def reset_colour_base_password(self):
        if self.cmb_color_question.get()=='' or self.val_color_answer.get()=='' or self.scvalue_color.get()=='':
            ...
            try:
                con = pymysql.connect(host="localhost", user="root", password="miki", database="employee3")
                cur = con.cursor()
                cur.execute("select * from student70 where email=%s and question=%s and answer=%s",
                ...
                if row == None:
                    ...
                    cur.execute("update student70 set ga_password=%s where email=%s",(self.reset_encrypted_color_password, self.val_email.get()))
                    con.commit()
                    con.close()
                    self.engine.say('Your password has been saved')
                    self.engine.runAndWait()
                    messagebox.showinfo("Success", "Your password has been saved",parent=self.root3)
                    self.root3.destroy()
    ...
    def colour_forget_password(self):
        if self.val_email.get() == "":
            self.engine.say('First Enter Your Email Address ')
            self.engine.runAndWait()
            messagebox.showerror("Error", "First Enter Your Email Address ", parent=self.window)
        else:
            try:
                con = pymysql.connect(host="localhost", user="root", password="miki", database="employee3")
                cur = con.cursor()
                cur.execute("select * from student70 where email=%s", self.val_email.get())
                row = cur.fetchone()
                if row == None:
                    self.engine.say('Please Enter The Valid Email Address')
                    self.engine.runAndWait()
                    ...
                    con.close()
                    self.root3 = Toplevel()
                    self.root3.title("Fill your saved details again!")
                    self.root3.geometry("750x590+750+300")
                    self.root3.config(bg="white")
                    self.root3.resizable(False, False)
                    self.root3.focus_force()
                    self.root3.grab_set()

                    title = Label(self.root3, text="Fill your saved details again!", font=("time new roman", 15, "bold"), fg="red",bg="white").place(x=7, y=10, relwidth=1)
                    ...
                    new_password = Label(self.root3, text="choose the color for confirmation", font=("time new roman", 15), bg="white",fg="gray").place(x=50, y=220)
                    ...
                    reset_password = Button(self.root3,command=self.reset_colour_base_password, text="Save the details!", bg="#d77337",fg="white",font=("time new roman", 12, "bold")).place(x=140, y=400, width=150, height=40)
    ...
    def colour_login(self):
        if self.val_email.get()=="":
            ...
            try:
                ...
                cur.execute("select * from student70 where email=%s", self.val_email.get())
                row=cur.fetchone()
                cur.execute("select * from student70 where email=%s and ga_password=%s",(self.val_email.get(), self.encrypted_color_password))
                row1 = cur.fetchone()
                if row==None:
                    self.engine.say('Please Enter The Valid Email Address')
                    self.engine.runAndWait()
                   ...
                   # self.clear2_text()
                    ...
                    self.engine.say('congratulation Colour base password Succesfuly login')
                    self.engine.runAndWait()
                    messagebox.showinfo("Success", "Colour base password Succesfuly login", parent=self.window)
                    self.clear2_text()
                    #self.window.destroy()
                    self.next_page()
                con.close()
    ...
    def next(self):
        ...
        bg_image = Image.open( r"D:/Graphical-Password-Authentication-System-master/pic/r.jpg")  # Replace with your image path
        bg_image = bg_image.resize((550, 540), Image.Resampling.LANCZOS)
        ...
        forget = Button(self.window, command=self.colour_forget_password, cursor="hand2",
                        text="Forget Password ?", bg="pink",
                        fg="black", bd=0, font=("time new roman", 14)).place(x=200, y=410)
...
# ********************************Third level of image base Security Page*************************************
        ...
        if self.cmb3_image_question.get()=='' or self.val3_image_answer.get()=='':
            ...
            try:
                ...
                if row == None:
                    ...
                    cur.execute("update student70 set gb_password=%s where email=%s",(self.reset_encrypted_image_password, self.val3_email.get()))
                    con.commit()
                    con.close()
                    self.engine.say('Your password has been saved')
                    self.engine.runAndWait()
                    messagebox.showinfo("Success", "Your password has been saved",parent=self.root4)
                    self.clear4_text()
                    self.root4.destroy()
            except Exception as es:
    ...
    def image_forget_password(self):
        if self.val3_email.get() == "":
            self.engine.say('Please Enter email address to save your password')
            self.engine.runAndWait()
            messagebox.showerror("Error", "Please Enter email address to save your password", parent=self.window1)
        else:
            try:
                con = pymysql.connect(host="localhost", user="root", password="miki", database="employee3")
                cur = con.cursor()
                cur.execute("select * from student70 where email=%s", self.val3_email.get())
                row = cur.fetchone()
                if row == None:
                    self.engine.say('Please Enter The Valid Email Address')
                    self.engine.runAndWait()
                    ...
                    con.close()
                    self.root4 = Toplevel()
                    self.root4.title("Confirm  Your Password")
                    ...
                    title = Label(self.root4, text="Fill your saved details again!", font=("time new roman", 12, "bold"), fg="red",bg="white").place(x=7, y=10, relwidth=1)
                    ...
                    new_password = Label(self.root4, text="Enter your saved Image base Password", font=("time new roman", 15), bg="white",fg="gray").place(x=70, y=180)
                    reset_password = Button(self.root4, command=self.reset_image_base_password, text="Save Password",
                                    bg="#d77337", fg="white", font=("time new roman", 12, "bold")).place(x=140, y=450,width=150,height=40)

                    self.bg102 = Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/image11111.jpg")
                    self.resizeA = self.bg102.resize((70, 70), Image.Resampling.LANCZOS)
                    self.photo102 = ImageTk.PhotoImage(self.resizeA)

                    self.bg22 = Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/image22222.jpg")
                    self.resizeB = self.bg22.resize((70, 70), Image.Resampling.LANCZOS)
                    self.photo22 = ImageTk.PhotoImage(self.resizeB)

                    self.bg33 = Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/image33333.jpg")
                    self.resizeC = self.bg33.resize((70, 70), Image.Resampling.LANCZOS)
                    self.photo33 = ImageTk.PhotoImage(self.resizeC)

                    self.bg44 = Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/image44444.jpg")
                    self.resizeD = self.bg44.resize((70, 70), Image.Resampling.LANCZOS)
                    self.photo44 = ImageTk.PhotoImage(self.resizeD)

                    self.bg55 = Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/image1111.jpg")
                    self.resizeE = self.bg55.resize((70, 70), Image.Resampling.LANCZOS)
                    self.photo55 = ImageTk.PhotoImage(self.resizeE)

                    self.bg66 = Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/image2222.jpg")
                    self.resizeF = self.bg66.resize((70, 70), Image.Resampling.LANCZOS)
                    self.photo66 = ImageTk.PhotoImage(self.resizeF)

                    self.bg77 = Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/image3333.jpg")
                    self.resizeG = self.bg77.resize((50, 50), Image.Resampling.LANCZOS)
                    self.photo77 = ImageTk.PhotoImage(self.resizeG)

                    self.bg88 = Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/image4444.jpg")
                    self.resizeH = self.bg88.resize((70, 70), Image.Resampling.LANCZOS)
                    self.photo88 = ImageTk.PhotoImage(self.resizeH)

                    self.bg99 = Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/image111.jpg")
                    self.resizeI = self.bg99.resize((70, 70), Image.Resampling.LANCZOS)
                    self.photo99 = ImageTk.PhotoImage(self.resizeI)

                    self.bg101 = Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/image222.jpg")
                    self.resizeJ = self.bg101.resize((70, 70), Image.Resampling.LANCZOS)
                    self.photo101 = ImageTk.PhotoImage(self.resizeJ)

                    self.bg111 = Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/image333.jpg")
                    self.resizeK = self.bg111.resize((70, 70), Image.Resampling.LANCZOS)
                    self.photo111 = ImageTk.PhotoImage(self.resizeK)

                    self.bg122 = Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/image444.jpg")
                    self.resizeL = self.bg122.resize((70, 70), Image.Resampling.LANCZOS)
    ...
    def image_login(self):
        if self.val3_email.get()=="":
            ...
            try:
                ...
                cur.execute("select * from student70 where email=%s", self.val3_email.get())
                row=cur.fetchone()
                cur.execute("select * from student70 where email=%s and gb_password=%s",(self.val3_email.get(), self.encrypted_image_password))
                row3 = cur.fetchone()
                if row==None:
                    self.engine.say('Please Enter The Valid Email Address')
                    self.engine.runAndWait()
                   ...
                   # self.engine.say('Invalid Password!')
                    ...
                    self.engine.say('Congratulation image-based password  succssfully login')
                    self.engine.runAndWait()
                    messagebox.showinfo("Success", "Image-base password Succssfully login", parent=self.window1)
                    self.clear3_text()
    ...
    def next_page(self):
        ...
        bg_label = Label(self.window1, image=self.bg_photo)
        bg_label.place(x=0, y=0, relwidth=1, relheight=1)
        ...
        self.bg1 = Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/image11111.jpg")
        self.resizeA = self.bg1.resize((100, 100), Image.Resampling.LANCZOS)
        self.photo1 = ImageTk.PhotoImage(self.resizeA)

        self.bg2 = Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/image22222.jpg")
        self.resizeB = self.bg2.resize((100, 100), Image.Resampling.LANCZOS)
        self.photo2 = ImageTk.PhotoImage(self.resizeB)

        self.bg3 = Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/image33333.jpg")
        self.resizeC = self.bg3.resize((100, 100), Image.Resampling.LANCZOS)
        self.photo3 = ImageTk.PhotoImage(self.resizeC)

        self.bg4 = Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/image44444.jpg")
        self.resizeD = self.bg4.resize((100, 100), Image.Resampling.LANCZOS)
        self.photo4 = ImageTk.PhotoImage(self.resizeD)

        self.bg5 = Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/image1111.jpg")
        self.resizeE = self.bg5.resize((100, 100), Image.Resampling.LANCZOS)
        self.photo5 = ImageTk.PhotoImage(self.resizeE)

        self.bg6 = Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/image2222.jpg")
        self.resizeF = self.bg6.resize((100, 100), Image.Resampling.LANCZOS)
        self.photo6 = ImageTk.PhotoImage(self.resizeF)

        self.bg7 = Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/image3333.jpg")
        self.resizeG = self.bg7.resize((100, 100), Image.Resampling.LANCZOS)
        self.photo7 = ImageTk.PhotoImage(self.resizeG)

        self.bg8 = Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/image4444.jpg")
        self.resizeH = self.bg8.resize((100, 100), Image.Resampling.LANCZOS)
        self.photo8 = ImageTk.PhotoImage(self.resizeH)

        self.bg9 = Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/image111.jpg")
        self.resizeI = self.bg9.resize((100, 100), Image.Resampling.LANCZOS)
        self.photo9 = ImageTk.PhotoImage(self.resizeI)

        self.bg10 = Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/image222.jpg")
        self.resizeJ = self.bg10.resize((100, 100), Image.Resampling.LANCZOS)
        self.photo10 = ImageTk.PhotoImage(self.resizeJ)

        self.bg11 = Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/image333.jpg")
        self.resizeK = self.bg11.resize((100, 100), Image.Resampling.LANCZOS)
        self.photo11 = ImageTk.PhotoImage(self.resizeK)

        self.bg12 = Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/image444.jpg")
        self.resizeL = self.bg12.resize((100, 100), Image.Resampling.LANCZOS)
        ...
        forget = Button(self.window1, command=self.image_forget_password, cursor="hand2", text="Forget Password ?",bg="white",
                        fg="red", bd=0, font=("time new roman", 12)).place(x=200, y=475)
    ...
    def student_details(self):
        ...
        self.bg122 = Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/student1.jpg")
        self.resizeL = self.bg122.resize((1590, 400), Image.Resampling.LANCZOS)
        self.photo122 = ImageTk.PhotoImage(self.resizeL)

        self.bg123 = Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/student2.jpg")
        self.resizeL = self.bg123.resize((1590, 400), Image.Resampling.LANCZOS)
