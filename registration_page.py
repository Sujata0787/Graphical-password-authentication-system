...
class signup:
    def __init__(self, root):
        self.color_encrypted_password = None
        self.root=root
        self.root.geometry("1600x800+0+0")
        self.root.maxsize(0,0)
        self.root.title("Registration Page")
        self.root.config(bg="#52595D")
        self.root.bg= Image.open(r"D:/Graphical-Password-Authentication-System-master/pic/imgregpic.jpg")
        new_size= (1600,800)
        img_resize=root.bg.resize(new_size)
        img_resize.save(r"D:/Graphical-Password-Authentication-System-master/pic/imgregpic.jpg"'')
        ...
        frame1.place(x=655,y=170, width=540, height=560)
    ...
    #---------------First name--------------------
     ...
     #----------------conform password--------------------
        ...
        conform_password=Label(self.root, text="Confirm Password",font=("time new roman", 15),bg="#2B547E",fg="#FFFFFF").place(x=970, y=400)
    ...
    def checkname(self,name):
        ...
        if name=='':
            ...
            self.engine.say('space is not allowed')
    ...
    def checkcontact(self, contact):
        ...
        if len(str(contact)) >= 0:
            ...
            self.engine.say('Character does not exit in phone number ')
            self.engine.runAndWait()
    ...
    def checkemail(self,email):
        if len(email)>7:
            if re.match("^[a-zA-Z0-9_.\-]+@gmail\.com$",email):
                return True
            else:
    ...
    def signup(self):
        if self.val_fname.get()=="":
            ...
            self.engine.say('Password & Confirm password should be same')
            self.engine.runAndWait()
            ...
            if self.val_chk.get()==0:
                ...
                try:
                    ...
                    if row!=None:
                        ...
                        cur.execute("INSERT INTO student70(f_name,l_name,email,contact,password,question,answer) values(%s,%s,%s,%s,%s,%s,%s)",
                                    (self.val_fname.get(),
                                     self.val_lname.get(),
                                     self.val_email.get(),
                        ...
                        con.commit()
                        con.close()
                        self.engine.say('Congratulation you are successfully registered ')
                        self.engine.runAndWait()
                        messagebox.showinfo("Success", "Text-based password successfully registered", parent=self.root)
                        self.clear1_text()
                        self.next()
    ...
    #*****************************************Second Label of colour base  Security_Page*****************************
        ...
        if self.scvalue.get()=="":
            ...
            try:
                if self.scvalue.get() !=None:
                    ...
                    self.val_email = StringVar()
                    self.email_entry = Entry(self.window, textvariable=self.val_email)
                ...
                con = pymysql.connect(host="localhost", user="root", password="miki", database="employee3")
                cur = con.cursor()
                cur.execute("""UPDATE student70 SET ga_password=%s WHERE email=%s""",
                            (self.color_encrypted_password, email))
                con.commit()  # ← This is essential!


                con.close()
                print(self.color_encrypted_password)
                print( self.val_email)
    ...
    def image_signup(self):
        if self.screen.get()=="":
            ...
            try:
                ...
                con=pymysql.connect(host="localhost",user="root",password="miki",database="employee3")
                cur = con.cursor()
                cur.execute("update student70 set gb_password=%s where email=%s",(self.image_encrypted_password,self.val_email.get()))
                con.commit()
                con.close()
...
#-----------------------------Main page of image base password-------------------
    def next_page(self):
        ...
        my_text3 = Label(self.window1, text="Image-based 3rd level of security", font=("time new roman", 15, "bold"), bg="white", fg="red").place(x=120, y=80)
